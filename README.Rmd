...
ggplot() +
 ...
 scale_fill_viridis_c(limits = c(0, 5))+
  geom_sf(data = analysis_point, color = "black", size = 1.5, shape = 21,
...
Once all the analysis methods have given a result, we need to be able to compare them. Notably, road access risk is not on the same 0 to 12 scale as the other three analyese. You might also want to add your own risk scores using other methods.
...
A linear transformation looks like this: 
...
summary_table <- tunisia |> 
  left_join(st_drop_geometry(rescaled), by = c("eu_id", "eu_name")) |> 
  left_join(st_drop_geometry(epi_units_entry_point_intro_risk), 
            by = c("eu_id", "eu_name"))

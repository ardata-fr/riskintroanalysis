...
vignette: >
  %\VignetteIndexEntry{Road access}
...
The road accessibility raster is provided by the `riskintrodata::download_road_access_raster()`.
...
Because the original scaling of the raw risk score is from 0 to around 1600, its 
hard to compare to other score. Here it is simply rescaled linearly to a range from 1 to 100 and reversed. The scale is reversed because the original raster 
data is based on distance from cities so the higher the number the further the location is from a city - the idea being that the more rural, the harder a place is to access, therefore having a lower risk of introduction of animal disease.
...
You can see the new scale displayed in the interactive map below.
...
plot_risk_interactive(ri_road_access_scaled)
```

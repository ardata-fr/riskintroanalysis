...
vignette: >
  %\VignetteIndexEntry{Border risk}
...
```{r shared-borders, warning=FALSE}
...
The map shows the shared borders as thick black lines with their lengths labeled in kilometers. Notice how one Tunisia governorates share borders with both Algeria and Libya, this is important
for weighting emission risk scores by border length.
...
- For units bordering **one country**: introduction risk = that country's emission risk
- For units bordering **multiple countries**: introduction risk = weighted average of emission risks, weighted by shared border length
...
The southernmost governorate (Remada) borders both Algeria and Libya, so its introduction risk is weighted. If it shares 60km with Algeria (emission risk = 8) and 40km with Libya (emission risk = 5), the calculation would be:

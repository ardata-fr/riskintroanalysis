---
title: "Analysis - Additional risks"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Analysis - Additional risks}
...
ggplot(ri_water) +
  ...
  labs(title = "Water course risk", fill = "Risk") +
  theme_minimal()
```
...
The sigmoid transformation creates an S-curve that compresses extreme values and expands middle values, useful when you want to dampen the effect of outliers.
...
## Guidelines for Additional Risks

When incorporating additional risks into your analysis:

1. **Document your sources** - clearly note where additional risk data comes from
2. **Choose appropriate aggregation** - consider whether mean, max, or sum best represents the risk
3. **Scale consistently** - ensure all risks use the same 0-100 scale before combining
4. **Select meaningful transformations** - linear, quadratic, exponential, and sigmoid each emphasize different parts of the risk distribution
5. **Validate spatial alignment** - verify that raster and vector data align correctly in space
...
ri_road_access <- tunisia |>
  mutate(road_access_risk = runif(n(), 0, 1600))
attr(ri_road_access, "risk_col") <- "road_access_risk"
attr(ri_road_access, "scale") <- c(0, 1600)
...
- **quadratic** - Emphasizes higher values (x²)
...
```{r interactive-map}
library(leaflet)
risk_mean <- summarise_scores(rt, method = "mean", keep_cols = TRUE)
plot_risk_interactive(risk_mean)
```

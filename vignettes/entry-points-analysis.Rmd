...
vignette: >
  %\VignetteIndexEntry{Entry points}
...
They correspond to places through which emission risk of animal diseases can become
a risk of introduction in epidemiological units. They typically correspond to border posts, or simply routes and paths that cross the border of the target territory, which are known to be used for animal transportation or transhumance.

They can also be interior (i.e. not in the border) locations such as airports or aerodromes.
But they are distinguished from the destinations of __animal movement__ in that:
...
For example, a border post with a sanitary control of transhumance herds can be considered a _controlled_ entry point.
...
![Schematic example of an epidemiological unit with two controlled and one not-controlled entry points.](img/diagram_entry-points.svg){#fig-diagram}

For each entry point, we first compute its value of _exposure_ $e$, which weights the number and emission scores $\nu_k$ of the source countries.
...
These values can be interpreted as an _effective number of __risky__ sources_. 
...
case_dat |>
  ...
  # filter(x == max(x))
  # filter(x == 2) |>
  # mutate(lab = paste(rep(c("𝛼 =", "𝛽 ="), each = 2), ab))
  ggplot(aes(x, risk)) +
  geom_line(
...
Here is a complete working example using Tunisia sample data to calculate entry point risk:
...
# Validate and prepare entry points dataset
...
# Validate and prepare epidemiological units
...
# Create emission risk factors for neighboring countries
